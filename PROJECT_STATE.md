...
8. **Czystość `functions.php`**: Dążymy do czystego pliku `functions.php` (docelowo ma tylko wywoływać skrypty przez `require_once` z folderu `inc/`), ALE robimy to **WYŁĄCZNIE MAŁYMI KROKAMI**. **BARDZO WAŻNE DLA ASYSTENTÓW AI:** Pod żadnym pozorem nie podejmuj się czyszczenia czy przenoszenia 3000 linii kodu na raz! Każdą funkcję modernizujemy i przenosimy do `inc/` pojedynczo, na wyraźne polecenie i w ramach danego zadania, aby uniknąć zepsucia krytycznych funkcjonalności.
9. **Moduł tłumaczeń JSON (Standard dla wszystkich bloków)**: We wszystkich modyfikowanych blokach (w pliku `edit.js`) implementujemy panel "Tłumaczenia AI (JSON)" w `InspectorControls`. Moduł ten pozwala wygenerować JSON ze wszystkimi tekstowymi atrybutami bloku, skopiować go, zlecić AI przetłumaczenie (np. na język niemiecki), a następnie wkleić z powrotem i automatycznie zaktualizować atrybuty. Znacznie to przyspiesza i ułatwia wprowadzanie niemieckich treści na nowym rynku DE. Jest to **obowiązkowy standard** przy każdym refactorze bloków.
10. **Responsive Spacing Control (Marginesy i Paddingi jak w Elementorze)**: W każdym nowym oraz refaktorowanym bloku należy implementować niestandardową kontrolkę marginesów i paddingów. W `block.json` dodajemy atrybuty: `marginDesktop`, `marginMobile`, `paddingDesktop`, `paddingMobile` (wszystkie typu `object` z `{ "default": {} }`). W `edit.js` ładujemy komponent `<ResponsiveSpacingControl attributes={attributes} setAttributes={setAttributes} />` do `InspectorControls`. Z kolei w `render.php` wstrzykujemy wygenerowane style za pomocą funkcji `require_once get_theme_file_path('/src/components/spacing-helper.php'); shav_render_responsive_spacing_css($block_id, $attributes);` tuż pod generowaniem unikalnego `$block_id`. Dodatkowo upewniamy się, że zewnętrzny kontener bloku (wrapper) posiada w klasach wygenerowany ciąg `$block_id`. Zapewnia to klientowi precyzyjną kontrolę nad odstępami.

## 10. Lekcje i dobre praktyki (Learnings z pracy)

1. **Build assets i WP Pusher (Bardzo Ważne):** Po uruchomieniu `npm run build` należy **zawsze pamiętać o dodaniu wygenerowanych plików z folderu `build/` do commita** (np. `build/style-index.css`, `build/<block>/index.js`, `build/<block>/index.asset.php`). Jeśli dodane zostaną tylko pliki źródłowe (`src/`, `css/`), WP Pusher zaktualizuje repozytorium, ale zmiany nie pojawią się na stronie na froncie.
2. **Wsteczna kompatybilność bloków (Fallbacks):** Kiedy przerabiamy twardo wbite atrybuty (np. linki polityki prywatności w stopce, albo statyczne `question1..10` w FAQ) na nowe dynamiczne rozwiązania (np. wybór menu, tablica obiektów), **zawsze zostawiamy stare pola jako fallback**. Dzięki temu obecne strony nie wybuchają ani nie tracą treści, jeśli klient nie wejdzie od razu do edytora żeby skonfigurować nową funkcjonalność.
3. **Migracja danych w edytorze (JS):** Przy zmianie struktury danych w `block.json` (np. przejście na tablicę `faqItems`), edytor (`edit.js`) powinien w locie wyłapywać stare atrybuty, tłumaczyć je na nową strukturę (array) i renderować nowy interfejs. Render PHP musi robić to samo dla bezpieczeństwa (jeśli tablica jest pusta, załaduj ze starych atrybutów).
4. **Layout i Flex-wrap na Tabletach (RWD):** Gdy dodajemy nowe sekcje do elastycznych kontenerów (jak social icons obok kolumn stopki) i brakuje nam miejsca, **nigdy nie wymuszamy `flex-wrap: nowrap` na siłę**, ponieważ to niszczy RWD na mniejszych tabletach (np. 768-1024px). Zamiast tego utrzymujemy `flex-wrap: wrap` dla bezpieczeństwa, a stylowanie elementu "spadającego" (np. ikon social) dostosowujemy tak (`flex-direction: row`), aby na nowej linii pod spodem ułożył się w estetyczny poziomy pasek.
5. **Szerokość bloków i unikanie przyklejania się do krawędzi (Responsive Margins):** Kiedy ograniczamy max-width głównego kontenera bloku do `1428px` (np. footer, header, banner) i chcemy uniknąć jego przyklejania się do lewej/prawej krawędzi przeglądarki na mniejszych ekranach, używamy wzorca `width: auto; margin: 0 20px;` dla widoku mobile, a dla desktopu (`min-width: $breakpoint-tablet`) ustawiamy `width: calc(100% - 40px); margin: 0 auto;`. Pozwala to na naturalne wyśrodkowanie przy dużych oknach bez zjawiska "overflow" ani dublowania paddingów. (Dodatkowo pamiętamy, by wyłączyć blok z listy pełnoszerokościowej `$shav-fullbleed-sections` w `utility-classes.scss` jeśli korzystał z wymuszonego margin/padding z tej klasy).
6. **Błąd w Gutenbergu "Witryna nie obsługuje bloku" (Unsupported block):** Pojawia się zazwyczaj wtedy, gdy działający w tle watcher `npm run blocks` ulegnie awarii, zawiesi się, bądź też w przeglądarce zapisze się uszkodzona/stara wersja skompilowanego pliku JS. **Rozwiązanie:** NIGDY nie klikaj opcji "Zmień na HTML" ani "Usuń" (spowoduje to utratę wprowadzonych danych bloku). Zamiast tego: 1. Uruchom w terminalu pełną ręczną kompilację: `npm run buildBlocks`. 2. Odśwież okno edytora WP wymuszając wyczyszczenie cache'u (Twardy reload: `Cmd + Shift + R`). Po tym zabiegu bloki ponownie zostaną poprawnie załadowane przez edytor.
