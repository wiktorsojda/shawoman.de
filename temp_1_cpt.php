...
// Wklej tutaj kod CPT z produkcji
/**
...
add_action('wp_ajax_blendygo_get_product_variations', 'blendygo_get_product_variations_ajax');
function blendygo_get_product_variations_ajax() {
    if ( ! current_user_can('edit_posts') ) {
    ...
    if ( $product_id <= 0 ) {
        wp_send_json_error('Błędne ID produktu');
    }
    
    $product = wc_get_product($product_id);
    if ( ! $product ) {
        wp_send_json_error('Nie znaleziono produktu');
    }
    ...
    if ( $product->is_type('variable') ) {
        ...
        foreach ( $variations as $var ) {
            ...
            foreach ( $var['attributes'] as $attr_key => $attr_value ) {
                ...
                $attr_list[] = $attr_label . ': ' . $attr_value_name;
                $attributes_raw[$attr_name] = strtolower($attr_value_name);
            }
            $attributes_string = implode(' | ', $attr_list);
            
            $results[] = array(
                'id' => $var['variation_id'],
                'title' => $product->get_name() . ' - ' . $attributes_string,
        ...
        $results[] = array(
            ...
            'attributes' => array(),
            'attributes_string' => 'Produkt prosty'
        );
    }
...
function render_promocje_meta_box($post)
{
    ...
    echo '<div class="promo-tab" data-tab="tab-gratis">Gratisy (Mapowanie)</div>';
    echo '</div>';
    ...
    $saved_prod_data = [];
    if (!empty($saved_prods)) {
        foreach ($saved_prods as $pid) {
            $saved_prod_data[] = ['id' => $pid, 'title' => get_the_title($pid)];
        }
    }
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        if ($set_target)
            echo '<option value="' . esc_attr($set_target) . '" selected="selected">' . esc_html(get_the_title($set_target)) . '</option>';
        echo '</select></div>';
        ...
        foreach ($where_prods_arr as $pid) {
            if ($pid)
                echo '<option value="' . esc_attr($pid) . '" selected="selected">' . esc_html(get_the_title($pid)) . '</option>';
        }
        echo '</select></div>';
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        if ($val && function_exists('wc_get_product')) {
            ...
            if ($p)
                echo '<option value="' . esc_attr($val) . '" selected="selected">' . esc_html(wp_strip_all_tags($p->get_formatted_name())) . '</option>';
        }
        echo '</select></div>';
    }
    echo '</div>';
    ...
    foreach ($drop_arr as $dpid) {
        if ($dpid && function_exists('wc_get_product')) {
            ...
            if ($p)
                echo '<option value="' . esc_attr(trim($dpid)) . '" selected="selected">' . esc_html(wp_strip_all_tags($p->get_formatted_name())) . '</option>';
        }
    }
    echo '</select></div>';
    echo '</div>';
    ...
    for ($d = 1; $d <= 5; $d++) {
        ...
        foreach ($disc_targets_arr as $dtid) {
            if ($dtid && function_exists('wc_get_product')) {
                ...
                if ($p)
                    echo '<option value="' . esc_attr(trim($dtid)) . '" selected="selected">' . esc_html(wp_strip_all_tags($p->get_formatted_name())) . '</option>';
            }
        }
        echo '</select></div>';
    ...
    // --- ZAKŁADKA GRATIS (MAPOWANIE) ---
    echo '<div id="tab-gratis" class="promo-tab-content">';
    echo '  <div class="promo-box">';
    echo '      <h3 style="font-size: 16px; margin-top: 0; padding-bottom: 10px; border-bottom: 1px solid #eee;">Mapowanie Gratisów</h3>';
    echo '      <p style="margin-bottom: 15px; color: #555;">Skonfiguruj parowanie konkretnych wyzwalaczy (produktów w koszyku) do wariantów gratisowych (dodawanych z ceną 0,00 zł).</p>';

    // Wykluczenia - przeniesione na górę
    echo '      <div style="background: #fff3f3; border: 1px solid #ffcccc; padding: 15px; border-radius: 5px; margin-bottom: 20px;">';
    echo '          <h4 style="margin-top: 0; margin-bottom: 10px; font-size: 14px; color: #d63638;">🔥 Wykluczenia z Gratisów</h4>';
    echo '          <p style="margin-bottom: 15px; font-size: 13px; color: #555;">Wybierz produkty (np. Zestawy), które NIGDY nie powinny wyzwalać darmowych nakrętek i gratisów.</p>';
    $excluded = get_post_meta($post->ID, 'promo_gratis_excluded', true);
    if (!is_array($excluded)) $excluded = array();
    echo '          <select class="wc-product-search" multiple="multiple" name="promo_gratis_excluded[]" style="width: 100%;" data-placeholder="Szukaj produktów do wykluczenia..." data-action="woocommerce_json_search_products_and_variations">';
    foreach ($excluded as $exc_id) {
        $exc_product = wc_get_product($exc_id);
        if ($exc_product) {
            echo '<option value="' . esc_attr($exc_id) . '" selected="selected">' . esc_html($exc_product->get_name()) . '</option>';
        }
    }
    echo '          </select>';
    echo '      </div>';
    ...
    // --- GENERATOR PAROWANIA ---
    echo '      <div style="background: #f0f6fc; border: 1px solid #c2dbf1; padding: 15px; border-radius: 5px; margin-bottom: 20px;">';
    echo '          <h4 style="margin-top: 0; margin-bottom: 10px; font-size: 14px; color: #2271b1;"><span class="dashicons dashicons-admin-generic" style="margin-top: 2px;"></span> Inteligentny Generator Parowania</h4>';
    echo '          <p style="margin-bottom: 15px; font-size: 13px; color: #555;">Wybierz produkty (z wariantami), a system automatycznie spróbuje sparować ich warianty po nazwach atrybutów.</p>';
    echo '          <div style="display: flex; gap: 15px; align-items: flex-end; margin-bottom: 15px;">';
    echo '              <div style="flex: 1;">';
    echo '                  <label style="display: block; font-weight: bold; margin-bottom: 5px; font-size: 13px;">1. Rodzic Wyzwalacza</label>';
    echo '                  <select class="wc-product-search" id="generator-trigger-parent" style="width: 100%;" data-placeholder="Szukaj produktu wyzwalającego..." data-action="woocommerce_json_search_products_and_variations"></select>';
    echo '              </div>';
    echo '              <div style="flex: 1;">';
    echo '                  <label style="display: block; font-weight: bold; margin-bottom: 5px; font-size: 13px;">2. Rodzic Gratisu</label>';
    echo '                  <select class="wc-product-search" id="generator-freebie-parent" style="width: 100%;" data-placeholder="Szukaj produktu gratisowego..." data-action="woocommerce_json_search_products_and_variations"></select>';
    echo '              </div>';
    echo '              <div>';
    echo '                  <button type="button" class="button button-primary" id="btn-generate-pairs" style="height: 30px; display: inline-flex; align-items: center; gap: 5px;"><span class="dashicons dashicons-update" style="font-size: 16px; width: 16px; height: 16px; margin-top: 3px;"></span> Pobierz i Generuj</button>';
    echo '              </div>';
    echo '          </div>';
    echo '          <div id="generator-results-area" style="display: none;">';
    echo '              <h4 style="margin-bottom: 10px; border-bottom: 1px solid #c2dbf1; padding-bottom: 5px;">Podgląd wygenerowanych par</h4>';
    echo '              <table class="wp-list-table widefat fixed striped" style="margin-bottom: 15px;">';
    echo '                  <thead><tr><th>Wariant wyzwalający (Atrybuty)</th><th>Wariant Gratis (Dopasowany)</th></tr></thead>';
    echo '                  <tbody id="generator-preview-list"></tbody>';
    echo '              </table>';
    echo '              <button type="button" class="button button-primary" id="btn-inject-pairs" style="background: #2271b1;"><span class="dashicons dashicons-yes-alt" style="margin-top: 3px;"></span> Zatwierdź i wstrzyknij do głównej listy</button>';
    echo '          </div>';
    echo '      </div>';

    echo '      <table class="form-table" style="max-width: 1000px; border-collapse: collapse; margin-bottom: 15px;">';
    echo '          <thead><tr style="text-align: left; background: #f9f9f9;"><th style="padding: 10px; border: 1px solid #ddd; width: 45%;">Wariant wyzwalający</th><th style="padding: 10px; border: 1px solid #ddd; width: 45%;">Wariant Gratis</th><th style="padding: 10px; border: 1px solid #ddd; width: 10%;">Akcje</th></tr></thead>';
    echo '          <tbody id="promo-gratis-mapping-list">';

    $gratis_mapping = get_post_meta($post->ID, 'promo_gratis_mapping', true);
    if (!is_array($gratis_mapping)) $gratis_mapping = array();

    if (!empty($gratis_mapping)) {
        foreach ($gratis_mapping as $idx => $map) {
            $trigger_id = intval($map['trigger']);
            $freebie_id = intval($map['freebie']);
            
            $trigger_obj = wc_get_product($trigger_id);
            if ($trigger_obj) {
                if ($trigger_obj->is_type('variation')) {
                    $parent = wc_get_product($trigger_obj->get_parent_id());
                    $p_name = $parent ? $parent->get_name() : $trigger_obj->get_name();
                    $attr_vals = [];
                    foreach ($trigger_obj->get_attributes() as $k => $v) {
                    ...
                    $trigger_name = $p_name . ' (' . implode(', ', $attr_vals) . ')';
                } else {
                    $trigger_name = $trigger_obj->get_name();
                }
            } else {
                $trigger_name = 'Brak produktu (ID: ' . $trigger_id . ')';
            }

            $freebie_obj = wc_get_product($freebie_id);
            if ($freebie_obj) {
                if ($freebie_obj->is_type('variation')) {
                    $parent = wc_get_product($freebie_obj->get_parent_id());
                    $p_name = $parent ? $parent->get_name() : $freebie_obj->get_name();
                    $attr_vals = [];
                    foreach ($freebie_obj->get_attributes() as $k => $v) {
                        if (taxonomy_exists($k)) {
                            $term = get_term_by('slug', $v, $k);
                            if ($term) { $v = $term->name; }
                        }
                    ...
                    $freebie_name = $p_name . ' (' . implode(', ', $attr_vals) . ')';
                } else {
                    $freebie_name = $freebie_obj->get_name();
                }
            } else {
                $freebie_name = 'Brak produktu (ID: ' . $freebie_id . ')';
            }
            
            echo '<tr class="promo-gratis-row">';
            echo '<td style="padding: 10px; border: 1px solid #ddd;"><select class="wc-product-search" name="promo_gratis_trigger[]" style="width: 100%;" data-action="woocommerce_json_search_products_and_variations"><option value="'.esc_attr($trigger_id).'" selected>'.esc_html($trigger_name).'</option></select></td>';
            echo '<td style="padding: 10px; border: 1px solid #ddd;"><select class="wc-product-search" name="promo_gratis_freebie[]" style="width: 100%;" data-action="woocommerce_json_search_products_and_variations"><option value="'.esc_attr($freebie_id).'" selected>'.esc_html($freebie_name).'</option></select></td>';
            echo '<td style="padding: 10px; border: 1px solid #ddd; text-align: center;"><button type="button" class="button remove-promo-gratis-row" style="color: #d63638; border-color: #d63638;">Usuń</button></td>';
            echo '</tr>';
        }
    }
    echo '          </tbody>';
    echo '      </table>';
    echo '      <p><button type="button" class="button button-secondary" id="add-promo-gratis-row" style="display: inline-flex; align-items: center; gap: 5px;"><span class="dashicons dashicons-plus" style="font-size: 16px; width: 16px; height: 16px; line-height: 1.5;"></span> Dodaj parę</button></p>';
    echo '  </div>';
    
    // Szablon JS dla nowego wiersza
    echo '  <script type="text/template" id="promo-gratis-row-template">';
    echo '      <tr class="promo-gratis-row">';
    echo '          <td style="padding: 10px; border: 1px solid #ddd;"><select class="wc-product-search" name="promo_gratis_trigger[]" style="width: 100%;" data-action="woocommerce_json_search_products_and_variations" data-placeholder="Wyszukaj wyzwalacz..."></select></td>';
    echo '          <td style="padding: 10px; border: 1px solid #ddd;"><select class="wc-product-search" name="promo_gratis_freebie[]" style="width: 100%;" data-action="woocommerce_json_search_products_and_variations" data-placeholder="Wyszukaj gratis..."></select></td>';
    echo '          <td style="padding: 10px; border: 1px solid #ddd; text-align: center;"><button type="button" class="button remove-promo-gratis-row" style="color: #d63638; border-color: #d63638;">Usuń</button></td>';
    echo '      </tr>';
    echo '  </script>';
    echo '</div>';
    ...
    foreach ($firm_colors as $fc) {
        echo '<div class="promo-swatch modal-swatch" style="width:20px;height:20px;background:' . $fc . ';" data-target="ctrl_c_num" data-color="' . $fc . '"></div>';
    }
    echo '                  </div>';
    ...
    foreach ($firm_colors as $fc) {
        echo '<div class="promo-swatch modal-swatch" style="width:20px;height:20px;background:' . $fc . ';" data-target="ctrl_c_lbl" data-color="' . $fc . '"></div>';
    }
    echo '                  </div>';
    echo '              </div>';
    ...
    <script>
        jQuery(document).ready(function ($) {
            ...
            // --- OBSŁUGA ZAKŁADKI GRATIS ---
            $('#add-promo-gratis-row').on('click', function() {
                var template = $('#promo-gratis-row-template').html();
                $('#promo-gratis-mapping-list').append(template);
                $(document.body).trigger('wc-enhanced-select-init');
            });
            $(document).on('click', '.remove-promo-gratis-row', function() {
                $(this).closest('tr').remove();
            });

            // --- LOGIKA GENERATORA PAROWANIA ---
            var generatorTriggerVars = [];
            var generatorFreebieVars = [];

            $('#btn-generate-pairs').on('click', function() {
                var triggerId = $('#generator-trigger-parent').val();
                var freebieId = $('#generator-freebie-parent').val();
                
                if (!triggerId || !freebieId) {
                    alert('Proszę wybrać oba produkty główne (Rodzic Wyzwalacza i Rodzic Gratisu).');
                    return;
                }

                $(this).text('Pobieranie...');
                $('#generator-results-area').hide();

                $.when(
                    $.post(ajaxurl, { action: 'blendygo_get_product_variations', product_id: triggerId }),
                    $.post(ajaxurl, { action: 'blendygo_get_product_variations', product_id: freebieId })
                ).done(function(res1, res2) {
                    $('#btn-generate-pairs').html('<span class="dashicons dashicons-update" style="font-size: 16px; width: 16px; height: 16px; margin-top: 3px;"></span> Pobierz i Generuj');
                    
                    if (res1[0].success && res2[0].success) {
                        generatorTriggerVars = res1[0].data;
                        generatorFreebieVars = res2[0].data;
                        
                        var html = '';
                        $.each(generatorTriggerVars, function(i, triggerVar) {
                            html += '<tr class="gen-preview-row" data-trigger-id="'+triggerVar.id+'" data-trigger-name="'+triggerVar.title+'">';
                            html += '<td><strong>'+triggerVar.title+'</strong></td>';
                            
                            var selectHtml = '<select class="gen-freebie-select" style="width:100%;">';
                            selectHtml += '<option value="">-- Wybierz (lub pozostaw puste aby pominąć) --</option>';
                            
                            var bestMatchId = '';
                            var maxScore = 0;
                            
                            var triggerVals = [];
                            $.each(triggerVar.attributes, function(t_key, t_val) {
                                triggerVals.push(t_val.toLowerCase());
                            });
                            
                            $.each(generatorFreebieVars, function(j, freebieVar) {
                                var score = 0;
                                
                                $.each(freebieVar.attributes, function(f_key, f_val) {
                                    if (triggerVals.indexOf(f_val.toLowerCase()) !== -1) {
                                        score += 5; // mają wspólną wartość atrybutu niezależnie od klucza
                                    }
                                });
                                
                                if (triggerVar.attributes_string === freebieVar.attributes_string && triggerVar.attributes_string !== '') {
                                    score += 10; 
                                }
                                
                                if (score > maxScore && score > 0) {
                                    maxScore = score;
                                    bestMatchId = freebieVar.id;
                                }
                            });
                            
                            $.each(generatorFreebieVars, function(j, freebieVar) {
                                var selected = (freebieVar.id == bestMatchId) ? 'selected' : '';
                                selectHtml += '<option value="'+freebieVar.id+'" '+selected+' data-name="'+freebieVar.title+'">'+freebieVar.title+'</option>';
                            });
                            selectHtml += '</select>';
                        ...
                        $('#generator-preview-list').html(html);
                        $('#generator-results-area').slideDown();
                    } else {
                        alert('Błąd pobierania wariantów z serwera.');
                    }
                }).fail(function() {
                    $('#btn-generate-pairs').html('<span class="dashicons dashicons-update" style="font-size: 16px; width: 16px; height: 16px; margin-top: 3px;"></span> Pobierz i Generuj');
            ...
            $('#btn-inject-pairs').on('click', function() {
                var injected = 0;
                $('#generator-preview-list .gen-preview-row').each(function() {
                    var triggerId = $(this).data('trigger-id');
                    var triggerName = $(this).data('trigger-name');
                    var selectEl = $(this).find('.gen-freebie-select');
                    var freebieId = selectEl.val();
                    var freebieName = selectEl.find('option:selected').data('name');
                    
                    if (freebieId && freebieId !== '') {
                        var template = $('#promo-gratis-row-template').html();
                        var $newRow = $(template);
                        
                        $newRow.find('select[name="promo_gratis_trigger[]"]').append('<option value="'+triggerId+'" selected>'+triggerName+'</option>');
                        $newRow.find('select[name="promo_gratis_freebie[]"]').append('<option value="'+freebieId+'" selected>'+freebieName+'</option>');
                        
                        $('#promo-gratis-mapping-list').append($newRow);
                        injected++;
                    }
                });
                ...
                if (injected > 0) {
                    $(document.body).trigger('wc-enhanced-select-init');
                    $('#generator-results-area').slideUp();
                    $('#generator-trigger-parent').val(null).trigger('change');
                    $('#generator-freebie-parent').val(null).trigger('change');
                    alert('Pomyślnie dodano ' + injected + ' par na listę. Nie zapomnij zaktualizować kampanii (Zaktualizuj), aby zapisać zmiany!');
                } else {
                    alert('Żadna para nie została wybrana, nic nie dodano.');
                }
            });
...
{
    if (!isset($_POST['promo_nonce']) || !wp_verify_nonce($_POST['promo_nonce'], 'promo_save_meta'))
        return;
    ...
    $text_fields = [
        ...
        'promo_priority',
        'promo_small_text',
        'promo_percentage_text',
        'promo_badge_bg',
        'promo_badge_color',
        'promo_hide_fse_btn',
        'promo_banner_desk',
        'promo_banner_mob',
        'promo_banner_desk_timer',
        'promo_banner_mob_timer',
        'promo_banner_shop_desk',
        'promo_banner_shop_mob',
        'promo_banner_cart_desk',
        'promo_banner_cart_mob',
        'promo_banner_main_desk',
        'promo_banner_main_mob',
        'timer_pos_top',
        'timer_pos_left',
        'timer_pos_top_mob',
        'timer_pos_left_mob',
        'timer_font_size_num',
        'timer_font_size_label',
        'timer_font_family',
        'timer_font_weight_num',
        'timer_font_weight_lbl',
        'timer_gap_col',
        'timer_gap_row',
        'timer_color_num',
        'timer_color_lbl',
        'promo_date_start',
        'promo_date_end',
        'promo_date_ext_start',
        'promo_date_final',
        'promo_date_final_fixed',
        'promo_date_remove_ui',
        'promo_photo_1',
        'promo_photo_mob_1',
        'promo_photo_2',
        'promo_photo_mob_2',
        ...
        'promo_photo_mob_5',
        'promo_badge_text_1',
        'promo_badge_icon_type_1',
        'promo_badge_image_1',
        'promo_badge_bg_color_1',
        'promo_badge_bg_image_1',
        'promo_badge_text_color_1',
        'promo_badge_width_1',
        'promo_badge_width_auto_1',
        'promo_badge_align_1',
        'promo_badge_mt_1',
        'promo_badge_mb_1',
        'promo_badge_py_1',
        'promo_badge_px_1',
        'promo_badge_icon_size_val_1',
        'promo_badge_icon_size_unit_1',
        'promo_badge_text_2',
        'promo_badge_icon_type_2',
        'promo_badge_image_2',
        'promo_badge_bg_color_2',
        'promo_badge_bg_image_2',
        'promo_badge_text_color_2',
        'promo_badge_width_2',
        'promo_badge_width_auto_2',
        'promo_badge_align_2',
        'promo_badge_mt_2',
        'promo_badge_mb_2',
        'promo_badge_py_2',
        'promo_badge_px_2',
        'promo_badge_icon_size_val_2',
        'promo_badge_icon_size_unit_2',
        'promo_badge_text_3',
        'promo_badge_icon_type_3',
        'promo_badge_image_3',
        'promo_badge_bg_color_3',
        'promo_badge_bg_image_3',
        'promo_badge_text_color_3',
        'promo_badge_width_3',
        'promo_badge_width_auto_3',
        'promo_badge_align_3',
        'promo_badge_mt_3',
        'promo_badge_mb_3',
        'promo_badge_py_3',
        'promo_badge_px_3',
        'promo_badge_icon_size_val_3',
        'promo_badge_icon_size_unit_3',
        'promo_fse_banner_image',
        'promo_fse_banner_image_mobile',
        'promo_fse_subtitle',
        'promo_fse_subtitle_color',
        'promo_fse_subtitle_bg',
        'promo_fse_subtitle_ff',
        'promo_fse_subtitle_fs',
        'promo_fse_title',
        'promo_fse_title_color',
        'promo_fse_title_ff',
        'promo_fse_title_fs',
        'promo_fse_title_fs_mobile',
        'promo_fse_title_icon_type',
        'promo_fse_title_svg_h_val',
        'promo_fse_title_svg_h_unit',
        'promo_fse_title_svg_hm_val',
        'promo_fse_title_svg_hm_unit',
        'promo_fse_title_svg_align',
        'promo_fse_body_text',
        'promo_fse_body_color',
        'promo_fse_body_ff',
        'promo_fse_body_fs',
        'promo_fse_button_text',
        'promo_fse_button_link',
        'promo_fse_button_bg',
        'promo_fse_button_text_color',
        'promo_fse_button_ff',
        'promo_fse_button_fs',
        'promo_fse_arrow_bg',
        'promo_fse_arrow_color',
        'promo_header_btn_enabled',
        'promo_header_btn_text',
        'promo_header_btn_link',
        'promo_mm_banner_enabled',
        'promo_mm_banner_image',
        'promo_mm_banner_subtitle',
        'promo_mm_banner_subtitle_bg',
        'promo_mm_banner_title',
        'promo_mm_banner_btn_text',
        'promo_mm_banner_btn_link',
        'promo_mm_banner_title_icon_type',
        'promo_mm_banner_title_svg_h_val',
        'promo_mm_banner_title_svg_h_unit',
        'promo_mm_banner_title_svg_align',
        'promo_mm_banner_title_image',
        'promo_mm_banner_subtitle_icon_type',
        'promo_mm_banner_subtitle_svg_h_val',
        'promo_mm_banner_subtitle_svg_h_unit',
        'promo_mm_banner_subtitle_svg_align',
        'promo_mm_banner_subtitle_image',
        'promo_mm_banner_btn_icon_type',
        'promo_mm_banner_btn_svg_h_val',
        'promo_mm_banner_btn_svg_h_unit',
        'promo_mm_banner_btn_svg_align',
        'promo_mm_banner_btn_image',
        'promo_aurora_product_1',
        'promo_aurora_product_2',
        'promo_aurora_product_3'
    ];
    ...
    $url_fields = [
        ...
        'promo_fse_banner_image',
        'promo_fse_banner_image_mobile',
        'promo_fse_title_image',
        'promo_mm_banner_image',
        'promo_mm_banner_title_image',
        'promo_mm_banner_subtitle_image',
        'promo_mm_banner_btn_image',
        'promo_banner_atc_desk',
        'promo_banner_atc_reg_url',
        'promo_fse_button_link',
        'promo_header_btn_link',
        'promo_mm_banner_btn_link'
    ];
    ...
    // ZAPIS MAPOWANIA GRATISÓW
    if (isset($_POST['promo_gratis_trigger']) && is_array($_POST['promo_gratis_trigger'])) {
        $gratis_mapping = array();
        foreach ($_POST['promo_gratis_trigger'] as $idx => $trigger_id) {
            $freebie_id = isset($_POST['promo_gratis_freebie'][$idx]) ? intval($_POST['promo_gratis_freebie'][$idx]) : 0;
            $trigger_id = intval($trigger_id);
            if ($trigger_id > 0 && $freebie_id > 0) {
                $gratis_mapping[] = array(
                    'trigger' => $trigger_id,
                    'freebie' => $freebie_id,
                );
        ...
        update_post_meta($post_id, 'promo_gratis_mapping', $gratis_mapping);
    } else {
        update_post_meta($post_id, 'promo_gratis_mapping', array());
    }

    if (isset($_POST['promo_gratis_excluded']) && is_array($_POST['promo_gratis_excluded'])) {
        $excluded = array_map('intval', $_POST['promo_gratis_excluded']);
        update_post_meta($post_id, 'promo_gratis_excluded', $excluded);
    } else {
        update_post_meta($post_id, 'promo_gratis_excluded', array());
    }
}
...
function blendygo_regulaminy_page_render()
{
    ...
    ?>
    <style>
        .blendygo-reg-container {
            max-width: 950px;
            margin-top: 15px;
        }

        .blendygo-reg-container .postbox {
        ...
        .reg-editor-wrap .wp-editor-wrap {
            width: 100%;
        }
    </style>
    ...
    <div class="wrap">
        ...
        <form method="post" action="" id="regulaminy-form" class="blendygo-reg-container">
            ...
            <div class="postbox" style="padding: 20px; border-left: 4px solid #2271b1;">
                ...
                <p style="color: #666; font-size: 13px;">Wybierz promocję, z której system pobierze kod rabatowy, daty
                    startu/zakończenia i wartość zniżki do aktywnego regulaminu. Na liście widoczne są tylko promocje o
                    statusie "Aktywna".</p>
            ...
            <div class="postbox" style="padding: 20px; display: flex; justify-content: space-between;">
                <div style="flex: 1;">
                    ...
                    <p style="margin-bottom: 15px; color: #555; font-size: 13px;">Kliknij w tag, aby skopiować go do
                        schowka, a następnie wklej w edytorze poniżej.</p>
                    <div style="display: flex; flex-wrap: wrap; gap: 8px;">
                        <code
                            style="cursor:pointer; padding:5px 10px; background:#f0f6fc; color:#2271b1; border-radius:3px;"
                            onclick="navigator.clipboard.writeText('{NAZWA}')" title="Kopiuj">{NAZWA}</code>
                        <code
                            style="cursor:pointer; padding:5px 10px; background:#f0f6fc; color:#2271b1; border-radius:3px;"
                            onclick="navigator.clipboard.writeText('{START}')" title="Kopiuj">{START}</code>
                        <code
                            style="cursor:pointer; padding:5px 10px; background:#f0f6fc; color:#2271b1; border-radius:3px;"
                            onclick="navigator.clipboard.writeText('{KONIEC}')" title="Kopiuj">{KONIEC}</code>
                        <code
                            style="cursor:pointer; padding:5px 10px; background:#f0f6fc; color:#2271b1; border-radius:3px;"
                            onclick="navigator.clipboard.writeText('{KOD}')" title="Kopiuj">{KOD}</code>
                        <code
                            style="cursor:pointer; padding:5px 10px; background:#f0f6fc; color:#2271b1; border-radius:3px;"
                            onclick="navigator.clipboard.writeText('{WARTOSC}')" title="Kopiuj">{WARTOSC}</code>
                    </div>
                </div>
                <div style="border-left: 1px solid #ddd; padding-left: 20px; width: 250px;">
                    ...
                    <code
                        style="display:block; text-align:center; padding:8px; background:#fcf0f1; color:#d63638; font-weight:bold; border-radius:3px;">[blendygo_regulamin]</code>
                </div>
            </div>
            ...
            <div id="regulaminy-lista">
                <?php foreach ($data['templates'] as $idx => $tpl): ?>
                    <div class="postbox reg-item" style="border-radius: 4px; overflow: hidden;">
                        <div
                            style="background: #f6f7f7; padding: 15px 20px; border-bottom: 1px solid #ccd0d4; display: flex; justify-content: space-between; align-items: center;">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                <label style="cursor: pointer; display: flex; align-items: center; gap: 5px;">
                                    ...
                                    <strong class="reg-radio-label"
                                        style="color: <?php echo ($data['active_template_idx'] == $idx) ? '#2271b1' : '#50575e'; ?>;">AKTYWNY
                                        SZABLON</strong>
                                </label>
                            </div>
                            <div style="display: flex; align-items: center; gap: 15px;">
                                <input type="text" name="tpl_name[]" value="<?php echo esc_attr($tpl['name']); ?>"
                                    placeholder="Nazwa robocza..." style="width: 250px; font-weight: bold; padding: 4px 8px;">
                                <button type="button" class="button toggle-reg-editor">Pokaż/Ukryj Edytor</button>
                                <button type="button" class="button remove-reg-item"
                                    style="color: #d63638; border-color: #d63638;">Usuń szablon</button>
                            </div>
                        </div>
                        <div class="reg-editor-wrap"
                            style="display: <?php echo ($data['active_template_idx'] == $idx) ? 'block' : 'none'; ?>;">
                            <?php
            ...
            <div
                style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px; margin-bottom: 40px;">
                ...
                <input type="submit" name="blendygo_save_regulaminy" class="button button-primary button-hero"
                    value="Zapisz zmiany w regulaminach">
            </div>
        </form>
    </div>
...
function blendygo_async_sideload_handler()
{
    ...
    if (empty($url) || empty($post_id) || empty($field_name))
