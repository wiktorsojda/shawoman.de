    ...
    function blendygo_get_label($key, $promo_id = 0)
    {
        ...
        $dictionary = [
            'pl' => [
                ...
                'mins' => 'minuty',
                'secs' => 'sekundy',
                'set_price' => 'Cena za zestaw:',
                'set_price_rest' => 'Cena poza zestawem:',
                'set_btn' => 'Zobacz zestaw',
            ...
            'hu' => [
                ...
                'secs' => 'másodperc',
                'set_price' => 'Szett ára:',
                'set_price_rest' => 'Szett nélküli ár:',
                'set_btn' => 'Szett megtekintése',
            ...
            'cs' => [
                ...
                'secs' => 'sekund',
                'set_price' => 'Cena sady:',
                'set_price_rest' => 'Běžná cena:',
                'set_btn' => 'Zobrazit sadu',
            ...
            'sk' => [
                ...
                'secs' => 'sekúnd',
                'set_price' => 'Cena setu:',
                'set_price_rest' => 'Bežná cena:',
                'set_btn' => 'Zobraziť set',
            ...
            'ro' => [
                ...
                'mins' => 'minute',
                'secs' => 'secunde',
                'set_price' => 'Prețul setului:',
                'set_price_rest' => 'Preț normal:',
                'set_btn' => 'Vezi setul',
            ...
            'it' => [
                ...
                'mins' => 'minuti',
                'secs' => 'secondi',
                'set_price' => 'Prezzo del set:',
                'set_price_rest' => 'Prezzo normale:',
                'set_btn' => 'Vedi il set',
            ...
            'nl' => [
                ...
                'secs' => 'seconden',
                'set_price' => 'Set prijs:',
                'set_price_rest' => 'Normale prijs:',
                'set_btn' => 'Bekijk de set',
...
if (!function_exists('blendygo_get_active_cpt_set_index')) {
    ...
    function blendygo_get_active_cpt_set_index($promo_id, $product_id = null)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            if (in_array($product_id, $allowed_prods))
                return $i; // DOKŁADNE DOPASOWANIE PRODUKTU - WYGRYWA

            $allowed_cats = explode(',', get_post_meta($promo_id, 'promo_set_where_cats_' . $i, true) ?: '');
            if (!empty(array_intersect($product_cats, $allowed_cats)))
                return $i; // DOKŁADNE DOPASOWANIE KATEGORII - WYGRYWA
...
<style>
            ...
            #blendygo-promo-banner-<?php echo $promo_id; ?> .countdown-timer-flex {
                display: flex;
                column-gap: <?php echo esc_attr($g_col); ?>px;
                font-family: <?php echo esc_attr($f_fam); ?>, sans-serif;
                height: 36px;
                line-height: normal;
                text-align: center;
            ...
            #blendygo-promo-banner-<?php echo $promo_id; ?> .countdown-wrapper {
                display: flex;
                flex-direction: column;
                align-items: center;
                row-gap: <?php echo esc_attr($g_row); ?>px;
            }

            #blendygo-promo-banner-<?php echo $promo_id; ?> .countdown-number {
                font-weight: <?php echo esc_attr($f_wei_n); ?>;
                font-size: <?php echo esc_attr($f_num); ?>px;
                <?php echo $c_num_style; ?>
            }

            #blendygo-promo-banner-<?php echo $promo_id; ?> .countdown-label {
                font-weight: <?php echo esc_attr($f_wei_l); ?>;
                font-size: <?php echo esc_attr($f_lbl); ?>px;
                text-transform: uppercase;
                line-height: 1;
                margin-top: 0px;
                <?php echo $c_lbl_style; ?>
            }

            @media (min-width: 769px) {

                .cpt-banner-mobile,
                .cpt-timer-mob-container {
                    display: none !important;
                }

                .cpt-timer-desk-container {
                    position: absolute;
                    top: <?php echo esc_attr($t_top); ?>%;
                    left: <?php echo esc_attr($t_left); ?>%;
            ...
            @media (max-width: 768px) {
                ...
                .cpt-timer-mob-container {
                    position: absolute;
                    top: <?php echo esc_attr($t_top_m); ?>%;
                    left: <?php echo esc_attr($t_left_m); ?>%;
                    transform: translate(-50%, -10%);
                    display: block !important;
                    width: 100%;
                    text-align: center;
        ...
        <div id="blendygo-promo-banner-<?php echo $promo_id; ?>" class="custom-banner-section"
            style="position: relative !important; display: block !important; width: 100%; text-align: center; margin-bottom: 20px;">
            <img src="<?php echo esc_url($img_dsktp); ?>" class="cpt-banner-desktop"
                style="width: 100%; height: auto; display: block;">

            <?php if ($is_grace_period): ?>
                <div class="cpt-timer-desk-container">
                    <div class="countdown-timer-flex"><span class="countdown-number"><?php echo esc_html($l_ending_soon); ?></span>
                    </div>
                </div>
                <div class="cpt-timer-mob-container">
                    <div class="countdown-timer-flex"><span class="countdown-number"><?php echo esc_html($l_ending_soon); ?></span>
                    </div>
                </div>
            <?php elseif ($use_timer && $remaining_seconds > 0): ?>
                <div class="cpt-timer-desk-container">
                    <div id="cpt-timer-desk-<?php echo $promo_id; ?>" class="countdown-timer-flex"></div>
                </div>
                <div class="cpt-timer-mob-container">
                    <div id="cpt-timer-mob-<?php echo $promo_id; ?>" class="countdown-timer-flex"></div>
                </div>
                <script>
                    (function () {
            ...
            <?php if ($img_mobi): ?><img src="<?php echo esc_url($img_mobi); ?>" class="cpt-banner-mobile"
                    style="width: 100%; height: auto;"><?php endif; ?>
        </div>
        <?php
    }
}
...
if (!function_exists('blendygo_render_cpt_atc_banner')) {
    function blendygo_render_cpt_atc_banner()
    {
        ...
        <style>
            ...
            @media (max-width: 768px) {
                .cpt-atc-banner .atc-img-desk {
                    display: none !important;
                }

                .cpt-atc-banner .atc-img-mob {
                    display: block !important;
                }
...
if (!function_exists('blendygo_render_cpt_cart_banner')) {
    function blendygo_render_cpt_cart_banner()
    {
        ...
        if ($img_dsktp)
            echo '<img class="cart-banner-image-desktop" src="' . esc_url($img_dsktp) . '" style="width: 100%; height: auto;">';
        if ($img_mobi)
            echo '<img class="cart-banner-image-mobile" src="' . esc_url($img_mobi) . '" style="width: 96%; margin: auto; height: auto; display:none;">';
        echo '</div>';
        ?>
        <style>
            @media (max-width: 768px) {
                .cpt-cart-banner .cart-banner-image-desktop {
                    display: none !important;
                }

                .cpt-cart-banner .cart-banner-image-mobile {
                    display: block !important;
                }
            }
        </style>
    ...
    function blendygo_override_fse_block_data($parsed_block)
    {
        if (isset($parsed_block['blockName']) && $parsed_block['blockName'] === 'ourblocktheme/urodzinybaner') {
            ...
            if ($promo_id) {
                ...
                // ZDJĘCIA BANERA
                if (!empty($meta['promo_fse_banner_image'][0]))
                    $parsed_block['attrs']['bannerImage'] = $meta['promo_fse_banner_image'][0];
                if (!empty($meta['promo_fse_banner_image_mobile'][0]))
                    $parsed_block['attrs']['bannerImageMobile'] = $meta['promo_fse_banner_image_mobile'][0];

                // PODTYTUŁ
                if (!empty($meta['promo_fse_subtitle'][0]))
                    $parsed_block['attrs']['subtitle'] = $meta['promo_fse_subtitle'][0];
                if (!empty($meta['promo_fse_subtitle_color'][0]))
                    $parsed_block['attrs']['subtitleColor'] = $meta['promo_fse_subtitle_color'][0];
                if (!empty($meta['promo_fse_subtitle_bg'][0]))
                    $parsed_block['attrs']['subtitleBgColor'] = $meta['promo_fse_subtitle_bg'][0];
                if (!empty($meta['promo_fse_subtitle_ff'][0]))
                    $parsed_block['attrs']['subtitleFontFamily'] = $meta['promo_fse_subtitle_ff'][0];
                if (isset($meta['promo_fse_subtitle_fs'][0]) && $meta['promo_fse_subtitle_fs'][0] !== '')
                    $parsed_block['attrs']['subtitleFontSize'] = (int) $meta['promo_fse_subtitle_fs'][0];

                // TYTUŁ
                if (isset($meta['promo_fse_use_title_svg'][0]))
                    $parsed_block['attrs']['useTitleSvg'] = ($meta['promo_fse_use_title_svg'][0] === 'yes');
                if (!empty($meta['promo_fse_title_svg'][0]))
                    $parsed_block['attrs']['titleSvg'] = $meta['promo_fse_title_svg'][0];
                if (isset($meta['promo_fse_title_svg_maxw'][0]) && $meta['promo_fse_title_svg_maxw'][0] !== '')
                    $parsed_block['attrs']['titleSvgMaxWidth'] = (int) $meta['promo_fse_title_svg_maxw'][0];
                if (isset($meta['promo_fse_title_svg_maxw_mobile'][0]) && $meta['promo_fse_title_svg_maxw_mobile'][0] !== '')
                    $parsed_block['attrs']['titleSvgMaxWidthMobile'] = (int) $meta['promo_fse_title_svg_maxw_mobile'][0];

                if (!empty($meta['promo_fse_title'][0]))
                    $parsed_block['attrs']['title'] = $meta['promo_fse_title'][0];
                if (!empty($meta['promo_fse_title_color'][0]))
                    $parsed_block['attrs']['titleColor'] = $meta['promo_fse_title_color'][0];
                if (!empty($meta['promo_fse_title_ff'][0]))
                    $parsed_block['attrs']['titleFontFamily'] = $meta['promo_fse_title_ff'][0];
                if (isset($meta['promo_fse_title_fs'][0]) && $meta['promo_fse_title_fs'][0] !== '')
                    $parsed_block['attrs']['titleFontSize'] = (int) $meta['promo_fse_title_fs'][0];
                if (isset($meta['promo_fse_title_fs_mobile'][0]) && $meta['promo_fse_title_fs_mobile'][0] !== '')
                    $parsed_block['attrs']['titleFontSizeMobile'] = (int) $meta['promo_fse_title_fs_mobile'][0];

                // TEKST OPISU
                if (!empty($meta['promo_fse_body_text'][0]))
                    $parsed_block['attrs']['bodyText'] = $meta['promo_fse_body_text'][0];
                if (!empty($meta['promo_fse_body_color'][0]))
                    $parsed_block['attrs']['bodyTextColor'] = $meta['promo_fse_body_color'][0];
                if (!empty($meta['promo_fse_body_ff'][0]))
                    $parsed_block['attrs']['bodyTextFontFamily'] = $meta['promo_fse_body_ff'][0];
                if (isset($meta['promo_fse_body_fs'][0]) && $meta['promo_fse_body_fs'][0] !== '')
                    $parsed_block['attrs']['bodyTextFontSize'] = (int) $meta['promo_fse_body_fs'][0];
                ...
                if (isset($meta['promo_hide_fse_btn'][0]) && $meta['promo_hide_fse_btn'][0] === 'yes') {
                    ...
                    if (!empty($meta['promo_fse_button_text'][0]))
                        $parsed_block['attrs']['buttonText'] = $meta['promo_fse_button_text'][0];
                    if (!empty($meta['promo_fse_button_link'][0]))
                        $parsed_block['attrs']['buttonLink'] = $meta['promo_fse_button_link'][0];
                    if (!empty($meta['promo_fse_button_bg'][0]))
                        $parsed_block['attrs']['buttonBgColor'] = $meta['promo_fse_button_bg'][0];
                    if (!empty($meta['promo_fse_button_text_color'][0]))
                        $parsed_block['attrs']['buttonTextColor'] = $meta['promo_fse_button_text_color'][0];
                    if (!empty($meta['promo_fse_button_ff'][0]))
                        $parsed_block['attrs']['buttonFontFamily'] = $meta['promo_fse_button_ff'][0];
                    if (isset($meta['promo_fse_button_fs'][0]) && $meta['promo_fse_button_fs'][0] !== '')
                        $parsed_block['attrs']['buttonFontSize'] = (int) $meta['promo_fse_button_fs'][0];
                    if (!empty($meta['promo_fse_arrow_bg'][0]))
                        $parsed_block['attrs']['arrowBgColor'] = $meta['promo_fse_arrow_bg'][0];
                    if (!empty($meta['promo_fse_arrow_color'][0]))
...
add_action('blendygo_cron_purge_cache', function () {
...
add_action('save_post_promocje', function ($post_id) {
    ...
    foreach ($dates as $d) {
        if ($d && strtotime($d) > current_time('timestamp'))
            wp_schedule_single_event(strtotime($d) + 30, 'blendygo_cron_purge_cache');
    }
    if (class_exists('LiteSpeed_Cache_API'))
        LiteSpeed_Cache_API::purge_all();
});
...
if (!function_exists('blendygo_cpt_dynamic_pricing')) {
    function blendygo_cpt_dynamic_pricing($price, $product)
    {
        ...
        $candidate_promos = [];
        if ($specific_promo)
            $candidate_promos[] = $specific_promo;
        if ($global_promo)
            $candidate_promos[] = $global_promo;
...
/**
 * =========================================================================
 * MODUŁ: GRATISY CPT (DODAWANIE W KOSZYKU)
 * =========================================================================
 */
add_action('woocommerce_cart_updated', 'blendygo_cpt_manage_gratis_logic');
function blendygo_cpt_manage_gratis_logic()
{
    ...
    $active_promos = [];
    foreach ($promos as $p) {
        if (blendygo_get_promo_phase($p->ID) > 0) {
            $active_promos[] = $p->ID;
        }
    }
    ...
    $all_mappings = [];
    $all_excluded = [];
    foreach ($active_promos as $pid) {
        $mapping = get_post_meta($pid, 'promo_gratis_mapping', true);
        ...
        $excluded = get_post_meta($pid, 'promo_gratis_excluded', true);
        if (is_array($excluded) && !empty($excluded)) {
            foreach ($excluded as $exc_id) {
                $all_excluded[] = intval($exc_id);
            }
        }
    }
    ...
    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        if (isset($cart_item['is_cpt_gratis']) && $cart_item['is_cpt_gratis'] && isset($cart_item['cpt_gratis_trigger_id'])) {
            $existing_gratis[$cart_item['cpt_gratis_trigger_id']] = array(
                'key' => $cart_item_key,
        ...
        $v_id = !empty($cart_item['variation_id']) ? $cart_item['variation_id'] : 0;

        // --- WYKLUCZENIA Z PANELU CPT ---
        $is_excluded = false;
        if (in_array($p_id, $all_excluded) || ($v_id > 0 && in_array($v_id, $all_excluded))) {
            $is_excluded = true;
        }
        if (isset($cart_item['woosg_parent_id']) && in_array(intval($cart_item['woosg_parent_id']), $all_excluded)) {
            $is_excluded = true;
        }
        if ($is_excluded) {
            continue;
        ...
        // Zostawiamy zestawy Smart Grouped, które nie są wykluczone w panelu CPT!
        // Twarda blokada wszystkich WPClever (woosg) została usunięta, aby polegać wyłącznie na panelu.


        // Zmienne $p_id i $v_id zostaly zdefiniowane wczesniej przy wykluczeniach

        if (!isset($triggers_in_cart[$p_id]))
            $triggers_in_cart[$p_id] = 0;
        $triggers_in_cart[$p_id] += $cart_item['quantity'];

        if ($v_id > 0) {
            if (!isset($triggers_in_cart[$v_id]))
                $triggers_in_cart[$v_id] = 0;
            $triggers_in_cart[$v_id] += $cart_item['quantity'];
        }
    }
    ...
    foreach ($all_mappings as $map) {
        ...
        if (isset($triggers_in_cart[$trigger_id]) && $triggers_in_cart[$trigger_id] > 0) {
            if (!isset($earned_freebies[$trigger_id])) {
                $earned_freebies[$trigger_id] = array('qty' => 0, 'freebie_id' => $freebie_id);
            }
            $earned_freebies[$trigger_id]['qty'] += $triggers_in_cart[$trigger_id];
        }
    }

    foreach ($earned_freebies as $trigger_id => $data) {
        $earned_qty = $data['qty'];
        $f_id = $data['freebie_id'];

        if (isset($existing_gratis[$trigger_id])) {
            if ($existing_gratis[$trigger_id]['qty'] != $earned_qty) {
                $cart->set_quantity($existing_gratis[$trigger_id]['key'], $earned_qty);
            }
            unset($existing_gratis[$trigger_id]);
        } else {
            if ($earned_qty > 0) {
                $cart->add_to_cart($f_id, $earned_qty, 0, array(), array('is_cpt_gratis' => true, 'cpt_gratis_trigger_id' => $trigger_id));
            }
        }
    }

    foreach ($existing_gratis as $trigger_id => $data) {
        $cart->remove_cart_item($data['key']);
    }

    $is_running = false;
}

add_action('woocommerce_before_calculate_totals', 'blendygo_cpt_modify_gratis_price', 99, 1);
function blendygo_cpt_modify_gratis_price($cart)
{
    if (is_admin() && !defined('DOING_AJAX'))
        return;

    foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
        if (isset($cart_item['is_cpt_gratis'])) {
            $currency = get_woocommerce_currency();
            $gratis_price = 0.01;
            if ($currency === 'HUF' || $currency === 'CZK') {
                $gratis_price = 1;
            }
            $cart_item['data']->set_price($gratis_price);
        }
    }
}

add_action('woocommerce_checkout_create_order_line_item', 'blendygo_cpt_add_gratis_meta_to_order', 10, 4);
function blendygo_cpt_add_gratis_meta_to_order($item, $cart_item_key, $values, $order)
{
    if (isset($values['is_cpt_gratis'])) {
        $item->add_meta_data('_is_gratis_item', 'yes');
    }
}add_action('wp_footer', function() {
    if (isset($_GET['debug_gratis']) && current_user_can('manage_options')) {
        echo '<div style="background: #fff; border: 2px solid red; padding: 20px; z-index: 99999; position: relative;">';
        echo '<h3>DEBUG KOSZYKA:</h3><pre>';
        foreach (WC()->cart->get_cart() as $key => $item) {
            echo "\n--- ITEM PRODUCT ID: {$item['product_id']} ---\n";
            $keys = array_keys($item);
            echo "Klucze: " . implode(', ', $keys) . "\n";
            if (isset($item['woosg_parent_id'])) echo "woosg_parent_id: {$item['woosg_parent_id']}\n";
            if (isset($item['bg_woosg_split'])) echo "bg_woosg_split: {$item['bg_woosg_split']}\n";
            $type = (isset($item['data']) && is_object($item['data'])) ? $item['data']->get_type() : 'unknown';
            echo "Typ produktu: {$type}\n";
        }
        echo '</pre></div>';
    }
});
