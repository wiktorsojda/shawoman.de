...
add_action('wp_footer', function () {
    ...
    <style>
        ...
        /* --- RESPONSOWNOŚĆ SZUFLADY --- */
        #dhl-packstation-section.dhl-active-drawer {
    ...
    <script>
        (function () {
            ...
            // NOWE: Funkcja ukrywająca/pokazująca sekcję i wpinająca ją w ramkę dostawy CheckoutWC
            window.initDhlPackstationVisibility = function() {
                var dhlSection = document.getElementById('dhl-packstation-section');
                if (!dhlSection) return;

                var shippingRadios = document.querySelectorAll('.shipping_method');
                if (shippingRadios.length === 0) return;

                var packstationRadio = null;

                // Szukamy radia, którego etykieta zawiera "Packstation" lub "Paketstation"
                shippingRadios.forEach(function(radio) {
                    var label = document.querySelector('label[for="' + radio.id + '"]');
                    if (label && label.innerText.match(/Packstation|Paketstation/i)) {
                        packstationRadio = radio;
                        
                        var liContainer = radio.closest('li');
                        if (liContainer && dhlSection.parentElement !== liContainer) {
                            liContainer.appendChild(dhlSection);
                            
                            // Stylowanie tak, by wyglądało jak rozsuwana szuflada
                            dhlSection.style.margin = '0';
                            dhlSection.style.border = 'none';
                            dhlSection.style.backgroundColor = '#ffffff';
                            dhlSection.style.borderRadius = '0 0 8px 8px';
                            dhlSection.style.width = '100%';
                            dhlSection.style.boxSizing = 'border-box';
                            // Dodajemy klasę, do której podepniemy responsywny padding w CSS
                            dhlSection.classList.add('dhl-active-drawer');
                        }
                    }
                });

                // Funkcja przełączająca widoczność
                function toggleDhlVisibility() {
                    if (packstationRadio && packstationRadio.checked) {
                        dhlSection.style.display = 'block';
                    } else {
                        dhlSection.style.display = 'none';
                    }
                }

                // Nasłuch na zmianę radio buttonów
                shippingRadios.forEach(function(radio) {
                    radio.addEventListener('change', toggleDhlVisibility);
                });

                // Pierwsze wywołanie by ukryć/pokazać na start
                toggleDhlVisibility();
            };
            ...
            if (typeof jQuery !== 'undefined') {
                // RATUNEK PRZED AJAX: Gdy CheckoutWC zaczyna przeładowywać metody wysyłki,
                // niszczy cały tag <ul>. Musimy ewakuować naszą sekcję w bezpieczne miejsce (do body),
                // by nie została usunięta z DOM!
                jQuery(document.body).on('update_checkout cfw_update_checkout', function() {
                    var dhlSection = document.getElementById('dhl-packstation-section');
                    if (dhlSection) {
                        dhlSection.style.display = 'none';
                        document.body.appendChild(dhlSection); // Bezpieczna przystań
                    }
                });

                // PO AJAX: CheckoutWC zbudował nowe <ul>. Przywracamy nasz skrypt, 
                // który pobierze naszą uratowaną sekcję z body i wepnie znów pod właściwe <li>.
                jQuery(document.body).on('updated_checkout cfw_updated_checkout', function() {
                    window.restoreDhlState();
                    window.initDhlPackstationVisibility();
                });
            }
